namespace xxdswinform.GapfillingDomain
{
    ...
    internal class G_two_2_5
    {
        private TestGenerateTools numberTools = new TestGenerateTools();
        private Random random = new Random();
        private XmlDocument xmlDocument = new XmlDocument();

        public void g_two_2_5Generate(NuGenEQML control1, string number)
        {
            this.xmlDocument.Load("GapfillingXML/g_two_2_5.xml");
            foreach (XmlNode node in this.xmlDocument.FirstChild.ChildNodes)
            {
                ...
                if (StudentModel.isReGeneration)
                {
                    ...
                    string path = "temp/generate/g_two_2_5.xml";
